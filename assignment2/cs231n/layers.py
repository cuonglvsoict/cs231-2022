...
def affine_forward(x, w, b):
    ...
    out = x.reshape(x.shape[0], -1).dot(w) + b.reshape(1, w.shape[1])
...
def softmax_loss(x, y):
    ...
    softmax = exp[range(exp.shape[0]), y] / sum_exp
    loss = np.sum(-np.log(softmax))
    loss /= x.shape[0]

    dx = exp / sum_exp.reshape(exp.shape[0], 1)
...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    d_var = np.sum(dx_hat * (x - mean), axis=0) * (-0.5) * (var + eps) ** (-1.5)
    d_mean = (-1.0 / np.sqrt(var + eps)) * np.sum(dx_hat, axis=0) - 2 * d_var * np.sum(x - mean, axis=0) / x.shape[0]

    dx = dx_hat * (1.0 / np.sqrt(var + eps)) + 2 * d_var * (x - mean) / x.shape[0] + d_mean / x.shape[0]
...
def dropout_backward(dout, cache):
    ...
    if mode == "train":
        ...
        dx = dout * (mask > 0) / dropout_param["p"]
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad_x = [[] for image in x]
    for i, image in enumerate(x):
      for channel in image:
        pad_x[i].append(np.pad(channel, ((pad, pad), (pad, pad)), "constant"))
    ...
    for n in range(N):
      for f in range(F):
        for h in range(Hp):
          for wp in range(Wp):
            for c in range(C):
              out[n, f, h, wp] += np.sum(pad_x[n, c, h*stride:h*stride+HH, wp*stride:wp*stride+WW] * w[f, c])
            out[n, f, h, wp] += b[f]
...
def conv_backward_naive(dout, cache):
    ...
    # this piece of code is copied from https://github.com/mantasu/cs231n/blob/master/assignment2/cs231n/layers.py
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
      for c in range(C):
        for h in range(Hp):
          for wp in range(Wp):
            out[n, c, h, wp] = np.max(x[n, c, h*stride:h*stride+pool_height, wp*stride:wp*stride+pool_width])
...
def max_pool_backward_naive(dout, cache):
    ...
    # this piece of code is also copied from https://github.com/mantasu/cs231n/blob/master/assignment2/cs231n/layers.py
