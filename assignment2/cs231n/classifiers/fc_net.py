...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, (x, y) in enumerate(zip(dims[:-1], dims[1:])):
          # print(i, x, y)
          self.params[f"W{i+1}"] = weight_scale * np.random.randn(x, y)
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers+1):
          ...
          gamma = self.params.get(f"gamma{layer}")
          beta = self.params.get(f"beta{layer}")
          reg_term += np.sum(w * w)

          norm_params = None
          dr_params = None
          if gamma is not None and layer < self.num_layers:
            norm_params = self.bn_params[layer-1]
            dr_params = self.dropout_param

          X, caches[layer] = forward(X, w, b, gamma, beta, norm_params, dr_params, layer==self.num_layers)
        ...
        for layer in range(self.num_layers, 0, -1):
          dout, dW, db, dgamma, dbeta = backward(dout, caches[layer])

          grads[f'W{layer}'] = dW + self.reg * self.params[f'W{layer}']
          grads[f'b{layer}'] = db
          # print(layer, dgamma)
          if dgamma is not None and layer < self.num_layers:
