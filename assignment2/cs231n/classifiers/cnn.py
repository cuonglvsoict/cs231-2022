...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        conv_out = 1 + (W + 2 * conv_pad - filter_size) // conv_stride
        pool_stride = 2
        pH = pW = 2
        pool_out = 1 + (conv_out - pH) // pool_stride

        self.params['W2'] = weight_scale * np.random.randn(F * pool_out * pool_out, hidden_dim)
